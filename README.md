...
- **LLM** - Local LLM inference server. Runs [llama.cpp](https://github.com/ggml-org/llama.cpp) as a rootless Podman quadlet under a dedicated `llm` user (UID 1060). Serves an OpenAI-compatible API (`/v1/chat/completions`) on host port 20600, accessed via Caddy by hostname. The model (Qwen 3.5 4B Q4_K_M) is auto-downloaded from HuggingFace on first boot and cached in `/var/local/lib/llm/models/`. By default the model uses chain-of-thought reasoning; clients can disable this per-request by passing `"chat_template_kwargs": {"enable_thinking": false}` in the API request body.
