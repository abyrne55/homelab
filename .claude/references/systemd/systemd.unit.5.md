...
Files (including directories) with names that match certain patterns are generally ignored. This includes names that start with a `.` or end with a `.ignore`.

The unit file format is covered by the [Interface Portability and Stability Promise](https://systemd.io/PORTABILITY_AND_STABILITY/).
...
| `/run/systemd/system.control` | Persistent and transient configuration created using the dbus API |
...
| `$XDG_RUNTIME_DIR/systemd/user.control` | Persistent and transient configuration created using the dbus API (`$XDG_CONFIG_HOME` is used if set, `~/.config` otherwise) |
...
Takes a value of `fail`, `replace`, `replace-irreversibly`, `isolate`, `flush`, `ignore-dependencies` or `ignore-requirements`. `OnFailureJobMode=` defaults to `replace`, `OnSuccessJobMode=` defaults to `fail`. Specifies how the units listed in `OnSuccess=`/`OnFailure=` will be enqueued. See `systemctl(1)`'s `--job-mode=` option for details on the possible values. If this is set to `isolate`, only a single unit may be listed in `OnSuccess=`/`OnFailure=`.
...
`ConditionPathIsSocket=` is similar to `ConditionPathExists=` but verifies that a certain path exists and is a socket.
...
| `PartOf=` | `ConsistsOf=` | [Unit] section, automatic property |
| `BindsTo=` | `BoundBy=` | [Unit] section, automatic property |
| `Requisite=` | `RequisiteOf=` | [Unit] section, automatic property |
| `Conflicts=` | `ConflictedBy=` | [Unit] section, automatic property |
| `Triggers=` | `TriggeredBy=` | Automatic properties, see notes below |
| `PropagatesReloadTo=` | `ReloadPropagatedFrom=` | [Unit] section |
| `ReloadPropagatedFrom=` | `PropagatesReloadTo=` | [Unit] section |
| `PropagatesStopTo=` | `StopPropagatedFrom=` | [Unit] section |
| `StopPropagatedFrom=` | `PropagatesStopTo=` | [Unit] section |
| `Following=` | n/a | Automatic property |
